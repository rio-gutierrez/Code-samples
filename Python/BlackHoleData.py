...
class NonlinearPoisson3D:
    ...
    def __init__(self, x, y, z):
        ...
        self.h = x[1] - x[0]      #equal grid spacing h = delta x = delta y = delta z
        ...
        for i in range(1, N - 1):
            for j in range(1, N - 1):
                for k in range(1, N - 1):
                    ...
                    self.A[index, index] = - (self.h ** 2) * f_prime[i, j, k] - 6.0
...
class InitialData:
        ...
        for i in range(1, N - 1):
            for j in range(1, N - 1):
                for k in range(1, N - 1):
                    ...
                    var_exp = self.theta[i, j, k] * (1.0 + self.u[i, j, k]) + 1.0
        ...
        for i in range(1, self.N - 1):
            for j in range(1, self.N - 1):
                for k in range(1, self.N - 1):
                    ...
                    f_in = self.theta[i, j, k] * (1.0 + self.u[i, j, k]) + 1.0 #expression inside parentheses in f
                    f = - self.rho[i, j, k]/(f_in ** 7)

                    self.res[i, j, k] = laplace - f        #R = laplace - f (recall Eq.(4.2))
                    res_norm += self.res[i, j, k] ** 2     #sum the square of all residuals
        ...
        for i in range(0, N):
            for j in range(0, N):
                for k in range(0, N):
                    ...
                    Aj_xy = J_coeff * (l_x * (j_z * l_x - j_x * l_z) + l_y * (j_y * l_z - j_z * l_y)  )
                    Aj_xz = J_coeff * (l_x * (j_x * l_y - j_y * l_x) + l_z * (j_y * l_z - j_z * l_y)  )
                    Aj_yz = J_coeff * (l_y * (j_x * l_y - j_y * l_x) + l_z * (j_z * l_x - j_x * l_z)  )

                    #Construct Bowen-York curvature with linear momentum A_p^{ij} (Eq. (3.22b))
                    P_coeff = 3.0/(2.0 * (r_bh**2))
                    lP = (l_x * p_x) + (l_y * p_y) + (l_z * p_z)     #contraction l_k P^k
                    Ap_xx = P_coeff * (2.0 * p_x * l_x + lP * (l_x * l_x - 1.0))
                    Ap_yy = P_coeff * (2.0 * p_y * l_y + lP * (l_y * l_y - 1.0))
                    Ap_zz = P_coeff * (2.0 * p_z * l_z + lP * (l_z * l_z - 1.0))
    ...
    def write_to_file(self):
        ...
        if out:
            ...
            print(" Could not open file ", filename," in write_to_file() function")
...
if __name__ == '__main__':
    ...
    main()
    print("It took %s seconds to run this beautiful code :)" % (time.time() - start_time))
